...
window.Globe = function(container, opts) {

    let colorFn = opts.colorFn;
    ...
    this.addData = function(data, opts) {
        ...
        let sub_geo = new THREE.Geometry();
        for (let i = 0, m = data.length; i < m; i += step) {
            lat = data[i];
            lng = data[i + 1];
            color = colorFnWrapper(data, i);
            size = data[i + 2];
            size = size * 200;
            addPoint(lat, lng, size, color, sub_geo);
        }
        if (opts.animated) {
            this._baseGeometry.morphTargets.push({'name': opts.name, vertices: sub_geo.vertices});
        } else {
            this._baseGeometry = sub_geo;
        }
    };
    ...
    function init() {
        ...
        container.addEventListener('mouseover', function() { overRenderer = true; }, false);
        container.addEventListener('mouseout', function() { overRenderer = false; }, false);
    ...
    function addPoint(lat, lng, size, color, sub_geo) {
        ...
        point.updateMatrix();
        for (let i = 0, m = point.geometry.faces.length; i < m; i++) {
            point.geometry.faces[i].color = color;
        }
        if(point.matrixAutoUpdate){
            point.updateMatrix();
        }
        sub_geo.merge(point.geometry, point.matrix);
    }
    ...
    function onDocumentKeyDown(event) {
        switch (event.keyCode) {
            case 38: zoom(100); event.preventDefault(); break;
            case 40: zoom(-100); event.preventDefault(); break;
        }
    }
